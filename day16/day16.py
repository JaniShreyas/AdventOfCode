...
def part2():
    l = len(data)
    l1 = len(data[0])
    curList: List[tuple[tuple[int,int], tuple[int,int]]] = [((0,j), (1,0)) for j in range(len(data[0]))]

    downList = [((l - 1, j), (-1,0)) for j in range(len(data[0]))]
    curList.extend(downList)
    
    rightList = [((i, 0), (0,1)) for i in range(len(data))]
    curList.extend(rightList)

    leftList = [((i, l1-1), (0,-1)) for i in range(len(data))]
    curList.extend(leftList)

    maxEnergised = 0
    for startSetting in curList:
        cur, dir = startSetting[0], startSetting[1]
        energised: dict[tuple, List[tuple]] = {cur: [dir]}
        paths: List[tuple[tuple, tuple]] = [(cur, dir)]
        ...
        energisedCount = len(energised)
        if energisedCount > maxEnergised:
            maxEnergised = energisedCount
    print(maxEnergised)
part2()